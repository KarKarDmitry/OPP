...
namespace OPP.Views.Pages.Guides
{
    ...
    {
        public static ObservableCollection<GlobalPackaging> GlobalPackagings { get; set; } = new ObservableCollection<GlobalPackaging>();
        public static ObservableCollection<GlobalLock> GlobalLocks { get; set; } = new ObservableCollection<GlobalLock>();
        public static ObservableCollection<GlobalAssembly> GlobalAssemblies { get; set; } = new ObservableCollection<GlobalAssembly>();
        public static ObservableCollection<GlobalPart> GlobalParts { get; set; } = new ObservableCollection<GlobalPart>();
        ...
        private void LoadGlobal()
        {
            Task.Run(async () =>
            {
                try
                {
                    if (GlobalPackagings.Count == 0)
                        await Task.Run(async () =>
                        {
                            await Dispatcher.InvokeAsync(() =>
                            {
                                foreach (var packaging in Packaging.GetAll())
                                {
                                    GlobalPackagings.Add(new GlobalPackaging(packaging));
                                }
                            });
                        });
        ...
        private void RootPackagingControl_MouseUp(object sender, MouseButtonEventArgs e)
        {
            var listBox = sender as DataGrid;
            var item = FindAncestor<DataGridRow>((DependencyObject)e.OriginalSource);
            ...
            {
                var selectedNode = listBox.ItemContainerGenerator.ItemFromContainer(item);
                OnNodeSelected(selectedNode, 1, PackagingGrid);
            }
        }
    }
